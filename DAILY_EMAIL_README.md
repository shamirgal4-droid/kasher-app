...
סקריפט שמדי בוקר בשעה שבחרת שולח לך מייל עם רשימת המשימות של היום.
הנושא של המייל: **"🌅 היום יש לך:"**
...
| `api/daily-email.py` | פונקציית Vercel שמופעלת ע"י ה-Cron |
| `vercel.json` | מגדיר מתי ה-Cron רץ |
...
## שלב 2א: הפעלה ב-Vercel (מומלץ, חינם)
...
ב-`vercel.json` ה-Cron מוגדר `"0 5 * * *"` — **Vercel רץ לפי UTC**.
- קיץ בישראל (IDT, UTC+3): `0 5 * * *` = **08:00** בבוקר ✅
- חורף בישראל (IST, UTC+2): שני את זה ל-`0 6 * * *` כדי שיישאר 08:00.
- רוצה שעה אחרת? חשבי: שעת ישראל פחות 3 (בקיץ) = שעת UTC.
